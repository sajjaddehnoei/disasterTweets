...
outputText <-
  inputText %>%
  text_vectorization() %>%
  layer_embedding(num_words, output_dim=256) %>%
...
outputOther <-
  inputOther %>%
  layer_dense(unit=256, activation="relu")
...
```{r lastLayer}
finalOutput <- concat %>%
  layer_dense(unit=2, activation="relu") %>%
  layer_dense(units = 1, activation = "sigmoid")
```

```{r buildModel}
model <- keras_model(inputs = list(inputText, inputOther), outputs = finalOutput)
```
